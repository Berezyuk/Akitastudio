...
class FfmpegHelper {
    ...
    public static function transcodeToH264(string $inputPath, string $mimeType): ?string {
        ...
        // быстрее, чем играет.
        //
        // scale='trunc(min(720,iw)/2)*2':-2 — короткая сторона не больше 720;
        //   min запрещает апскейл мелких исходников. -2 у высоты выравнивает
        //   чётность только вычисляемой стороны — ширина задана явным
        //   выражением min(720,iw) и без trunc(...)/2*2 чётности не получает:
        //   если исходник уже ⩽720 и нечётной ширины (например 641), min
        //   вернёт её как есть, yuv420p требует чётности по обеим осям, и
        //   ffmpeg падает ("width not divisible by 2"). trunc(x/2)*2 округляет
        //   вниз до чётного перед тем, как -2 посчитает высоту под него.
        ...
        $cmd = sprintf(
            'ffmpeg -i %s -vf "scale=\'trunc(min(720,iw)/2)*2\':-2" -c:v libx264 -profile:v high '

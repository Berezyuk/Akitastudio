...
### 11.5 Деплой и миграции схемы

Боевой стек живёт за **хостовым nginx** (вне докера), который терминирует TLS и
проксирует: `/api/` → `127.0.0.1:8000`, `/` → `127.0.0.1:3001` (статика `dist/`),
`media.<домен>` → `127.0.0.1:9000` (MinIO). Эталон — `docs/nginx-host.conf`.
Compose 80/443 не занимает.

**Обновление прода — одной командой:**

```bash
make prod
```

Что делает `make prod`:
1. `prerender` — `vite build` + пре-рендер HTML для SEO (`dist/`).
2. `docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d --build`
   (`--build` обязателен: `php.ini` **копируется** в образ, без пересборки правки
   не доедут — молча).
3. Ждёт готовности PostgreSQL и прогоняет `make migrate`.

**Миграции схемы — критично.** `docker/postgres/init.sql` применяется ТОЛЬКО на
свежий volume (`CREATE TABLE IF NOT EXISTS` пропускает изменения на существующей
БД). Все правки схемы на живой БД доезжают лишь через `docker/postgres/migrations/*.sql`.

- `make prod` прогоняет их автоматически (шаг 3). Все миграции **идемпотентны**
  (`IF NOT EXISTS` / guard'ы), повторный прогон на каждом деплое безопасен.
- Прогнать вручную (например, на уже поднятом стенде без полного `make prod`):
...
> **Почему это важно:** код полагается на схему. Например, создание заказа пишет
> в колонку `orders.idempotency_key` и рассчитывает на UNIQUE-констрейнты и CHECK.
> Задеплоить код без соответствующей миграции = 500 на каждом заказе. Порядок:
> сначала миграции (или `make prod`, который делает это сам), потом код полагается
> на новую схему.

**Новая миграция:** положить файл `docker/postgres/migrations/ГГГГ-ММ-ДД_описание.sql`
(идемпотентный: `ADD COLUMN IF NOT EXISTS`, `CREATE INDEX IF NOT EXISTS`,
`DROP CONSTRAINT IF EXISTS` перед `ADD CONSTRAINT`). Продублировать изменение и в
`init.sql` — чтобы свежий стенд поднимался уже с ним. `make migrate` подхватит по
имени (сортировка по алфавиту → префикс датой).

### 11.6 CI

`.github/workflows/ci.yml` на каждый PR/push в `main` гоняет: юниты (`vitest`),
сборку фронта (`vite build`), синтаксис PHP (`php -l`). HTTP-смоук
(`tests/api.test.js`) в CI НЕ входит — ему нужен поднятый стек с БД/MinIO.
Авто-деплой (CD) не настроен: нужен SSH-доступ к серверу в secrets репозитория.

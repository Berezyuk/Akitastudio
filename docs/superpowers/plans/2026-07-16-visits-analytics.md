...
### Task 5: Проверка антинакрутки — ОТМЕНЕНА (поглощена фиксом Task 4)

Задача предписывала тест потолка на 30 визитов в час по `visitor_hash`. Ревью
Task 4 показало, что такой потолок вообще не работает: корзина была привязана
к паре (IP, UA), и `curl` с ротацией `User-Agent` обходил его целиком.

Фикс Task 4 (коммит `1c3b7aa`) развёл хеши, поднял потолок до 300/час по
`ip_hash` и накрыл это тестом, который гоняет настоящие 300 запросов и ротирует
UA внутри заполняющего цикла — то есть ловит ровно ту дыру, мимо которой прошла
исходная формулировка. Тест прошёл обратную проверку: возврат `withinLimit()` на
`visitor_hash` красит его точечно.

Писать поверх этого исходный тест на 30 нечего: он проверял бы константу,
которой больше нет.
